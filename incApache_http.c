...
/*** TO BE DONE 4.0 START ***/
            ...
            strcat(http_header, "\r\nSet-Cookie: ");
            strcat(http_header, "UserID=");
            sprintf(http_header + strlen(http_header), "%d;", cookie);
...
/*** TO BE DONE 4.0 START ***/
                    ...
                    while(option_val != NULL && *option_val == ' ')
                        option_val++;
                    if(option_val != NULL && strncmp(option_val, "UserID=", strlen("UserID=")) == 0) {
                        sscanf(option_val + strlen("UserID="), "%d", &UIDcookie);
                        fprintf(stderr, "\t\t... %d", UIDcookie);
...
/*** TO BE DONE 4.0 START ***/
                    ...
                    option_name = strtok_r(NULL, "\r\n", &strtokr_save);

                    if (    option_name != NULL &&
                            strcmp(option_name, "If-Modified-Since") == 0 &&
                            (option_val = strtok_r(NULL, "\r\n", &strtokr_save)) != NULL &&
                            strptime(option_val, " %a, %d %b %Y %T GMT", &since_tm) != NULL
                            )
...
/*** TO BE DONE 4.0 START ***/
                ...
                if(difftime(my_timegm(&since_tm), (stat_p->st_mtim).tv_sec) <= 0)
